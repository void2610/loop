...
# OKF 知識バンドル(§6)。anchor = ROOT(loop engine checkout)。engine=公開 / data=非公開。
KNOWLEDGE_DIRNAME = "Knowledge"
ENGINE_KNOWLEDGE = ROOT / KNOWLEDGE_DIRNAME
DATA_KNOWLEDGE = DATA / KNOWLEDGE_DIRNAME
...
# --- 知識バンドル(OKF・§6)の解決・書き込みガード・コミット段 ---

def knowledge_bundles(cfg: dict) -> list[Path]:
    """[knowledge].bundles を anchor(ROOT)起点で絶対化して返す(存在するバンドルのみ・重複除去)。
    `<data-dir>` トークンは解決済み DATA に展開。未指定なら engine + data の 2 バンドル(§6.2/§6.3)。"""
    spec = cfg.get("knowledge", {}).get("bundles")
    if spec:
        cands = []
        for raw in spec:
            s = str(raw).replace("<data-dir>", str(DATA))
            p = Path(s)
            cands.append(p if p.is_absolute() else (ROOT / p))
    else:
        cands = [ENGINE_KNOWLEDGE, DATA_KNOWLEDGE]
    seen: set[Path] = set()
    out: list[Path] = []
    for p in cands:
        rp = p.resolve()
...
def build_knowledge_brief(cfg: dict) -> str:
    """役(Author/Implementer)に知識バンドルの在り処を anchor 解決済み絶対パスで渡す(種類A)。
    非公開バンドル(judgment を含む)は read-only・判断を真似ない、を明示する(§6.1/§7)。"""
    bundles = knowledge_bundles(cfg)
    if not bundles:
        return ""
    lines = "\n".join(f"- `{b}`" for b in bundles)
    return ("\n\n# 知識バンドル(OKF・任意参照)\n"
            "次の OKF 知識バンドルを read で参照できます(anchor 解決済みの絶対パス)。"
            "各バンドルの `index.md` を起点に、タスクに関連する概念だけを progressive disclosure で辿ってください。"
            "読む前に engine 側の `conventions/CONVENTIONS.md` と `conventions-loop.md`(規約)に従うこと。\n"
            f"{lines}\n"
            "非公開バンドル(`decisions/` を含む data 側)は**読むだけ**。"
            "そこにある人間の判断(種類B)を真似て新たな判断を生成しないこと。")


def _knowledge_is_human(path: Path) -> bool:
    """種類B ガード: `decisions/` 配下、または frontmatter に `judgment: human` を持つファイルは人間専用。"""
    ...
    try:
        ...
        return False
    return bool(re.search(r"(?m)^judgment:\s*human\s*$", head))


def _changed_knowledge(repo: Path, bundle: Path) -> list[Path]:
    """repo 内 bundle 配下で git 的に変更のある .md(種類A のみ)。judgment:human / decisions/ は除外。
    run が触れたパスの厳密追跡は書き込みが絶対パス経由で困難なため、バンドルの git status 走査で代替する。"""
    try:
        rel = bundle.resolve().relative_to(repo.resolve())
    except ValueError:
        return []
    # -uall: 新規ファイルをディレクトリに畳まず個別に列挙(run が新概念 .md を作る場合に拾うため)
    p = git(repo, "status", "--porcelain", "-uall", "--", str(rel))
    if p.returncode != 0:
        return []
    out: list[Path] = []
    for line in p.stdout.splitlines():
        name = line[3:].strip()
        if " -> " in name:  # rename は「旧 -> 新」
            name = name.split(" -> ", 1)[1]
        f = repo / name
        if f.suffix == ".md" and f.exists() and not _knowledge_is_human(f):
            out.append(f)
    return out


def commit_knowledge_writes() -> None:
    """種類A の知識更新を確定する(§6.4c)。既存 auto_commit は知識を拾わないため新設。
    data 側は _DATA_COMMIT_LOCK 下で commit。engine 側は knowledge: 接頭辞で commit するが**自動 push しない**
    (public は混入確認後に人手で push)。judgment:human / decisions/ は除外(種類B は人間が別途コミット)。"""
    cfg = load_config()
    bundles = {b.resolve() for b in knowledge_bundles(cfg)}
    if DATA_KNOWLEDGE.resolve() in bundles:
        paths = _changed_knowledge(DATA, DATA_KNOWLEDGE)
        if paths:
            auto_commit(DATA, paths, "knowledge: run が触れた学び/事実を確定(種類A)")
    if ENGINE_KNOWLEDGE.resolve() in bundles:
        paths = _changed_knowledge(ROOT, ENGINE_KNOWLEDGE)
        if paths:
            auto_commit(ROOT, paths, "knowledge: run が触れた設計知識を確定(種類A・push は人間)")


def cmd_knowledge_link() -> int:
    """engine バンドル直下に gitignored symlink `Knowledge/private -> <data-dir>/Knowledge` を張る(§6.4b)。
    既存 symlink は張り替え。実ディレクトリが居座る場合は事故防止で触らず手動確認を促す。"""
    ENGINE_KNOWLEDGE.mkdir(parents=True, exist_ok=True)
    DATA_KNOWLEDGE.mkdir(parents=True, exist_ok=True)
    link = ENGINE_KNOWLEDGE / "private"
    if link.is_symlink():
        link.unlink()  # 壊れリンク含め張り替え
    elif link.exists():
        print(f"  · {link} が symlink でない実体です。手動確認が必要", file=sys.stderr)
        return 1
    link.symlink_to(DATA_KNOWLEDGE)
    print(f"  · linked {link} -> {DATA_KNOWLEDGE}")
    return 0


def cmd_knowledge_commit(paths: list[str]) -> int:
    """loop外セッションの知識更新を 2 repo 分離を守って commit する(§6.4c)。
    各パスを resolve(symlink 実体化)し data 配下 / engine 配下を判定して正しい repo へ knowledge: 接頭辞で commit。
    engine は自動 push しない。人間経路なので種類B(decisions/ / judgment:human)も commit してよい。"""
    if not paths:
        print("usage: runner.py knowledge-commit <path>...", file=sys.stderr)
        return 2
    eng_paths: list[Path] = []
    data_paths: list[Path] = []
    for raw in paths:
        p = Path(raw).resolve()  # symlink Knowledge/private/... は実体(DATA 配下)へ解決される
        if not p.exists():
            print(f"  · 存在しない: {p}", file=sys.stderr)
            continue
        if _is_under(p, DATA):  # DATA は ROOT 配下なので DATA を先に判定する
            data_paths.append(p)
        elif _is_under(p, ROOT):
            eng_paths.append(p)
        else:
            print(f"  · engine/data いずれの配下でもない: {p}", file=sys.stderr)
    if data_paths:
        with _DATA_COMMIT_LOCK:
            auto_commit(DATA, data_paths, "knowledge: 知識を更新")
        print(f"  · data に commit: {len(data_paths)} 件")
    if eng_paths:
        auto_commit(ROOT, eng_paths, "knowledge: 知識を更新")
        print(f"  · engine に commit: {len(eng_paths)} 件(自動 push しない。git status 確認後に人手で push)")
    return 0
...
def build_author_prompt(prompt: str, cfg: dict, repo_path: Path | None) -> dict:
    ...
    knowledge = build_knowledge_brief(cfg) if inspect else ""
    brief = constitution + norms + repo_brief + knowledge
...
def _finalize_run(task: dict, run_id: str, run_dir: Path, md: Path, final: str, commit_msg: str) -> None:
    ...
    with _DATA_COMMIT_LOCK:
        ...
        commit_knowledge_writes()  # 種類A の知識更新を確定(§6.4c)。judgment:human / decisions/ は除外
...
def _run_attempt(task: dict, run_id: str, cfg: dict, started_at: str) -> tuple[str, bool]:
    ...
    try:
        ...
        brief = build_constitution_brief() + build_norms_brief(repo, cfg) + build_repo_brief(repo, int(loop.get("repo_history_runs", 8))) + build_knowledge_brief(cfg)
...
def main() -> int:
    ...
    if cmd == "knowledge-link":
        return cmd_knowledge_link()
    if cmd == "knowledge-commit":
        return cmd_knowledge_commit(sys.argv[2:])
    ...
    print("unknown command: {}\nusage: runner.py [run [task_id]|gen <prompt> [--run]|reindex|status|merges|"
          "knowledge-link|knowledge-commit <path>...|"
