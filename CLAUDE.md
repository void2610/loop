...
data/(別 private repo / engine からは .gitignore / 複数 PC で共有):
  hosts/<hostname>/      各 PC 専用ディレクトリ。loop.local.toml [data] dir で指す
    tasks/<id>.md  tasks/plans/<id>.md(Author 生成の実装プラン)
    runs/<id>.md + runs/<id>/  review-notes.md  plans/(設計メモ)  loop.db
```
...
> - **promote 段**: run=pass の成果を必ず PR 化し、**GitHub CI + Copilot レビュー**が green になるまで Implementer を差し戻して回す(種類A)。個別タスクで PR を出したくないときは task に `no_pr: true` を付ける(他に分岐はない)。green でも **`pass` にせず `awaiting-merge`** で確定(真の完了は人間の PR マージ後)。**merge は人間(自動 merge しない)**。上限超過は handoff。
...
- **検証・テスト実行はトークンを消費しても確認を取らず自動でやってよい**(ユーザー許可済み)。実装の正しさは実際に動かして証拠で示すのが優先。
- **E2E / ライブ run による検証はコスト確認不要・トークン無制限で実行してよい**(2026-06-23 ユーザー明示許可)。新機能(配線・runner 変更等)を入れたら、確認を取らずに実 repo への E2E run まで回して証拠で示す。「たぶん動く」「ライブ run はコスト確認が要るので指示待ち」で止めない。
- **大きめの本番投入(大量 run・破壊的操作)だけは必要性とコストを一言添えて確認**。検証目的の run はこれに含めない(上記のとおり無確認で回す)。
