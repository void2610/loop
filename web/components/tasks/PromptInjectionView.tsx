...
export function PromptInjectionView({ taskId, host }: { taskId: string; host?: string }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <PromptSection title="① 憲法 (constitution.md / 最優先・不可侵)" body={data.constitution} />
      <PromptSection title="② このリポジトリの設計規範 (conventions.md / 人間が承認済み)" body={data.norms} />
      <PromptSection
      ...
      />
      <PromptSection title="④ タスク契約 (goal / accept / constraints / verify)" body={data.task_contract} />
      <PromptSection title="⑤ Author の実装プラン (tasks/plans/<id>.md)" body={data.author_plan} />
      <PromptSection
