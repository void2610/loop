...
種類B には 2 つの局面がある。どちらも GUI が代行・生成しない:
- **run 後のレビュー判断**(R&D): 上記の「信用できるか/学び/入れるべきチェック」。
- **run 中の操作判断**(実装中に方針/権限で詰まったときの決定、PR の merge)。エージェントが `NEEDS_HUMAN` で
  止まり、人間が Web から続行指示を注入する/PR をマージして初めて run が真に完了する。**最終確定は人間**。

唯一の硬い制約: **GUI は判断を生成・要約・推奨・自動入力しない**。事実要約(runner が作る)と証拠表示まで。
verdict の妥当性・信用度・学び、そして実装中の決定や merge は人間が行う。
