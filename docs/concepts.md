...
| **Sub-agents** | Explorer / Implementer / Verifier の3役分離。Verifier は別モデル必須 |
| **Connectors** | ローカル FS / git / shell。証拠はローカルテスト実行から |
...
Bash は絶対パスで worktree 外も触れる。外部FS を触るタスク(例: `~/Downloads` 整理 = `repo: none`)の安全性は、
隔離ではなく**タスクの constraints**(バックアップ先行・削除禁止・件数不変 verify など)で担保する。
read-only 役(Explorer/Verifier)と生成は `--disallowedTools` で変更系ツールを禁止し、global settings
