...
max_attempts: "1"        # (任意・quoted) run 全体の再試行上限。非冪等タスク(外部FS書き換え等)は "1"
...
> **YAML の型に注意**: `max_attempts` は **quoted string("1")** で書く。unquoted の `max_attempts: 1` は YAML が int でパースし、
> `TaskFields(str)` の pydantic v2 strict が弾いて TaskDetail API が 500 になる(`runner.write_task` / `fm_from_form` は
> 保存時に str 化するので、Web/Author 経由で作る分には自動だが、手書きや既存資産の MD は quoted を徹底する)。
