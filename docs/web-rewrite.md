...
> 1=全体アーキテクチャと repo 構成 / 2=API レイヤ設計 / 3=リアルタイム監視・並行 run(SSE) / 4=runner の並列実行(別トラック T)/ 5=分析ダッシュボード / 6=レビュー(種類B)UX / 7=リモートアクセス・認証・セキュリティ / 8=段階的移行・超並列実装計画。
>
> 注意: 各セクション本文中の散文での相互参照(「§5 を参照」「セクション6 に委譲」等)は、並列設計時の各担当の暫定番号が混在しており**上の正準番号とズレている箇所がある**(例: 本文が「§5(SSE)」「§6(認証)」「§7(ダッシュボード)」と書く箇所の正準は 3/7/5)。リンクを辿る際は上の正準番号・目次を基準にすること。
